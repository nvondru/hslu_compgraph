...
function SolidSphere(gl, latitudeBands, longitudeBands, color) {
  ...
  var verticesAndTextures = defineVerticesAndTexture(
    latitudeBands,
    longitudeBands
  );
  ...
  gl.bufferData(
    gl.ARRAY_BUFFER,
    new Float32Array(verticesAndTextures.vertices),
    gl.STATIC_DRAW
  );
  ...
  gl.bufferData(
    gl.ARRAY_BUFFER,
    new Float32Array(verticesAndTextures.normals),
    gl.STATIC_DRAW
  );
  ...
  gl.bufferData(
    gl.ARRAY_BUFFER,
    new Float32Array(verticesAndTextures.textures),
    gl.STATIC_DRAW
  );
  ...
  sphere.scale = function (multiplier) {
    sphere.width += 0.1 * multiplier;
    sphere.height += 0.1 * multiplier;
    sphere.depth += 0.1 * multiplier;
  };
  sphere.rotate = function (xRotation, yRotation) {
    sphere.angle += xRotation;
    sphere.angle += yRotation;
  };
  return sphere;
}
